...
class zipimporter:
    ...
    def __init__(self, path, *args):
        ...
        if len(args) > 0 and isinstance(args[0], bytes):
            self.is_qt = True
            self.zip_bytes = args[0]
            prefix = args[1].split("/")[::-1]
        else:
        ...
        try:
            ...
            if self.is_qt:
                files = _read_directory(path, args[0])
            else:
    ...
    def get_data(self, pathname):
        ...
        try:
            ...
            raise OSError(0, '', key)
        return _get_data(self.archive, toc_entry, self=self)
    ...
    def get_source(self, fullname):
        ...
        try:
            ...
            return None
        return _get_data(self.archive, toc_entry, self=self).decode()
...
def _read_directory(archive, *args):
    try:
        if len(args) > 0:
            fp = _io.BytesIO(args[0])
        else:
...
def _get_data(archive, toc_entry, self=None):
    datapath, compress, data_size, file_size, file_offset, time, date, crc = toc_entry
    if data_size < 0:
        raise ZipImportError('negative data size')

    with (_io.BytesIO(self.zip_bytes) if self and self.is_qt else _io.open_code(archive)) as fp:
...
def _get_module_code(self, fullname):
    ...
    for suffix, isbytecode, ispackage in _zip_searchorder:
        ...
        try:
            ...
            modpath = toc_entry[0]
            data = _get_data(self.archive, toc_entry, self=self)
            if isbytecode:
